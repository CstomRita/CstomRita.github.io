...
如果是成员变量，那么不分基本类型和引用类型都是在java的堆内存里面分配空间，
...
![类加载器层次关系图](JVM%E5%86%85%E5%AD%98%E6%A8%A1%E5%9E%8B.assets/class-loader-parents-delegation-model.png)
...
我们知道两个类是否相等取决于两个类来自同一个class文件+被同一个加载器加载。 
