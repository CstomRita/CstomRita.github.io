...
> [!Note]
>
> - RANGE分区根据指定列的范围进行分区，如果是日期、数值等具有范围特性的列考虑RANGE分区表。
>
> - LIST分区根据某一列的值将数据分区，如果是类别、地区等具有离散特性的列，考虑LIST分区。
>
> - HASH分区通过计算哈希函数将数据分区，支持用户自定义哈希函数，具有一定的随机值，当需要各个分区数据相对平均，没有明显顺序关系，可以随机排列时，考虑HASH分区。
>
> - KEY分区是通过mysql自带的hash函数将数据分区，支持字符串等非整型作为分区键，适用场景与hash类似，当需要对字符串等类型分区时可考虑KEY分区
...
根据某一列的范围值将数据分布到不同的分区。每个分区包含特定的范围。
...
HASH分区是使用哈希算法将数据均匀地分布到多个分区中，支持用户自定义哈希函数。
...
KEY分区是根据某一列的哈希值将数据分布到不同的分区。不同于HASH分区，KEY分区使用的是mysql自带的hash函数。
...
1. KEY分区支持除text和BLOB之外的所有数据类型的分区，而HASH分区只支持数字分区；
2. KEY分区使用系统提供的HASH函数进行分区，HSAH分区允许使用用户自定义的表达式进行分区。
