...
在主从的层面上，Kafka副本分为leader副本和follow副本，生产者只会把数据发往Leader，然后Follower找Leader进行同步数据。

从是否与leader保持同步层面上，和Leader保持同步的Follower集合称为ISR；长时间未向Leader发送通信请求或同步数据的副本集合称为OSR；所有分区的副本为AR，AR=ISR+OSR。
...
Kafka集群中，常见的leader副本选举策略有三种：

1、首选副本策略。在这种策略下，每个分区都有一个首选副本，通常是副本集合中的第一个副本。当触发选举时，控制器会优先选择该首选副本作为新的Leader。

2、ISR副本选举策略。按照AR顺序，挑选首个在ISR的副本作为新的leader。

3、UncleanLeader选举策略。当所有ISR副本都不可用时，会从所有副本中，包括OSR中选择一个副本作为新的Leader副本，即使这个副本与当前Leader副本不同步，这种选举策略可能会导致数据丢失，一般不使用。

### 副本leader选举时机？

在以下场景下会触发leader副本的选举：

1、当前leader副本出现故障或失去连接，或者当前leader副本所在的broker宕机

2、新增broker时：新增Broker时，触发选举，以重新分配Partition的Leader。

3、新建分区时：当一个新的分区被创建时，需要选举一个Leader。

4、ISR副本数量小于副本因子时，为了保证数据的安全性，就会触发Leader选举。
...
如果持续拉取速度慢于leader副本写入速度，慢于时间超过副本同步最大时间后，它就变成“非同步”副本，就会被踢出ISR副本集合中。
...
消息队列常见的使用场景比较核心的有 3 个：**解耦**、**异步**、**削峰**。

需要注意的问题包括：

- 消息队列的高可用性，系统引入的外部依赖越多，需要额外保障依赖的可用性。
- 消息队列的可靠性，需要保障消息消息未丢失、消息顺序性等问题。
- 消息消费的一致性问题，保障发送和消费消息的一致性。
...
1、对于生产者，可以通过修改批处理大小、缓冲区大小、压缩参数等提高吞吐量。

2、对于消费者：

- 如果是Kafka消费能力不足，则可以考虑增加Topic的分区数，并且同时提升消费组的消费者数量。
- 如果是下游的业务数据处理不及时，批次拉取数据过少，处理的数据小于生产的数据，也会造成数据积压，这种情况通过提高每批次拉取的数量来解决。

### 如何保障消息的唯一性/避免重复消息和漏消费？

1、对于生产者，通过开启幂等性配置。Kafka会通过会话ID、分区ID、主键的组合判断数据是否重复，Producer不论向Broker发送多少次重复数据，Broker端都只会持久化一条，保证了不重复。

2、对于生产者，已经消费了数据，但是offset没提交会造成重复消费；先提交offset后消费，有可能会造成数据的漏消费；如果想完成Consumer端的精准一次性消费，那么需要Kafka消费端将消费过程和提交offset过程做原子绑定。
...
这种情况很有可能是消费者挂掉了，但如果积压了几百万到上千万的数据，即使消费者恢复了，也需要很长的时间才能恢复，因此需要临时紧急扩容。

1. 先修复 consumer 的问题，然后将现有 consumer 都停掉。
2. 新建一个 topic，partition 是原来的 10 倍，临时建立好原先 10 倍的 queue 数量。
3. 然后写一个临时的分发数据的 consumer 程序，这个程序部署上去消费积压的数据，**消费之后不做耗时的处理**，直接均匀轮询写入临时建立好的 10 倍数量的 queue。
4. 临时征用 10 倍的机器来部署 consumer，每一批 consumer 消费一个临时 queue 的数据。这种做法相当于是临时将 queue 资源和 consumer 资源扩大 10 倍，以正常的 10 倍速度来消费数据。
5. 快速消费完积压数据之后，**得恢复原先部署的架构**，**重新**用原先的 consumer 机器来消费消息。

### 生产过程中何时会发生QueueFullExpection以及如何处理/Kafka中消息快慢了如何处理？
